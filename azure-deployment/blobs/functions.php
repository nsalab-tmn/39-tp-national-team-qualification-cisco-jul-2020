...
function updateDatabase($db) {
	...
	// Permissions table
	try {
		$query = "select table_name as name  from information_schema.TABLES where TABLE_SCHEMA=\"eve_ng_db\" and table_name = 'permissions' ;";
//		$query = "SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'permissions';";
		...
		if ($result['name'] != 'permissions') {
			// User table is missing
			$db -> beginTransaction();
			$query = 'CREATE TABLE permissions (lab_id TEXT, role TEXT, username TEXT, PRIMARY KEY (lab_id(32), role(32), username(32)));';
			$statement = $db -> prepare($query);
			$statement -> execute();
